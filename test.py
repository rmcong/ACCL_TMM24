...
def parse_option():
    ...
    parser.add_argument('--target_task', type=int, default=19, help='resume task')
...
def test():
    opt = parse_option()
    dataloader = datagenerator.DatasetGen(opt)
    ...
    discriminator = get_discriminator(opt, task_id=target_task)
    discriminator = load_discriminator(discriminator, target_task, opt)
    ...
    for u in range(opt.target_task+1):
        ...
        test_res = eval_(data_loader=test_loader, task_id=u, model=test_model, discriminator=discriminator,
                         opt=opt)

        print('>>> Test on task {:2d} - {:15s}: loss={:.3f}, acc={:5.1f}% <<<'.format(u, test_loader_name,
                                                                                      test_res['loss_t'],
                                                                                      test_res['acc_t']))
        acc[target_task-1, u] = test_res['acc_t']
        lss[target_task-1, u] = test_res['loss_t']
