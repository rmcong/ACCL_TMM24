...
class MiniImageNet(torch.utils.data.Dataset):
    ...
    def __init__(self, root, train):
        ...
        with open(os.path.join(root,'{}_data.pkl'.format(self.name)), 'rb') as f:
    ...
    def __getitem__(self, i):
        img, target = self.data[i], self.labels[i]
        return img, target
...
class Memory(torch.utils.data.Dataset):
    ...
    def __init__(self, memory, memory_classes, task_num):
        super(Memory, self).__init__()
        self.train_data = []
        self.train_global_labels = []
        self.train_labels = []
        self.train_tt = []
        self.train_td = []
        self.memory_classes = memory_classes
...
class DatasetGen(object):
    ...
    def get(self, task_id):
        ...
        # self.memory_loader = Memory(memory, memory_classes, task_id)
        # memory_bank = torch.utils.data.DataLoader(self.memory_loader, batch_size=self.batch_size, num_workers=self.num_workers,
        ...
        if self.use_memory:
            ...
            # weights = np.array([0.] * len(train_split.indices))
            # ut, uc = np.unique(self.train_set[task_id].train_global_labels, return_counts=True)
            # weight_list = np.array([0.]) * len(ut)
            # for t, c in zip(ut, uc):
            # for i in range(len(train_split.indices)):
            #     c = uc[np.where(ut == self.train_set[task_id][train_split.indices[i]][1])[0]]
            #     if c > 200:
            #         weights[i] = 1./c * (5*c/len(train_split.indices))
            #     else:
            #         weights[i] = 1./c * (len(reply_indices)/len(train_split.indices))
            # train_sampler = data.WeightedRandomSampler(torch.Tensor(weights), len(weights))

        train_loader = torch.utils.data.DataLoader(train_split, batch_size=self.batch_size, num_workers=self.num_workers,
                                                   pin_memory=self.pin_memory, shuffle=True,)#sampler=train_sampler)
        ...
        # self.dataloaders[task_id]['memory'] = memory_bank
    ...
    def get_train_and_memory(self, task_id):
    ...
    def get_all_test_dataloader(self):
        ...
        self.test_set = iMiniImageNet(root=self.root, classes=list(np.arange(100)),memory_classes=None, task_num=19,
                                           memory=None, train=False, transform=self.transformation)
